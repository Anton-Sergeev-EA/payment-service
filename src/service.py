...
class PaymentService:
    ...
    async def create_operation(
        ...
        async with db.get_connection() as conn:
            ...
            # Insert operation.
            #
            # The SELECT check above is inherently racy under concurrent
            # requests: two coroutines can both pass it (neither has
            # inserted yet) and then both attempt this INSERT. Relying
            # solely on the pre-check was the actual bug — under
            # concurrency it let a raw sqlite3.IntegrityError escape as
            # an unhandled exception (500) instead of the intended 409
            # Conflict. The database's UNIQUE constraint on
            # operation_id is the real source of truth for
            # "does this exist"; catching its violation here is what
            # makes create_operation correctly idempotent under
            # concurrent identical requests, not just sequential ones.
            try:
                await conn.execute(
    ...
    async def submit_operation(self, operation_id: str) -> Tuple[Operation, bool]:
        ...
        async with db.get_connection() as conn:
            ...
            try:
                # Get operation — BEGIN IMMEDIATE above already holds a write lock
                cursor = await conn.execute(
                    "SELECT * FROM operations WHERE operation_id = ?",
                    (operation_id,)
                )
                ...
                # Update to PROCESSING atomically with rowcount check
                now = datetime.now(timezone.utc).isoformat()
                ...
                # CRITICAL FIX #1: Check if update actually changed anything
                ...
                # Add event - ONLY if we successfully updated
                await conn.execute(
                    """INSERT INTO events 
                       (operation_id, event_type, from_status, to_status, message, occurred_at)
                       VALUES (?, ?, ?, ?, ?, ?)""",
    ...
    async def process_receipt(self, receipt_data: Dict) -> bool:
        ...
        async with db.get_connection() as conn:
            ...
            try:
                # Check if receipt already processed with lock.
                cursor = await conn.execute(
                ...
                if operation.status in [OperationStatus.COMPLETED, OperationStatus.REJECTED]:
                    logger.warning(
                        f"IGNORED receipt for operation {receipt.operation_id}: "
                        f"already final {operation.status}, received {receipt.result}",
                        extra={
                            "operation_id": receipt.operation_id,
                            "existing_status": operation.status.value,
                            "new_result": receipt.result,
                            "provider_payment_id": receipt.provider_payment_id
                        }
                    )
                    
                    # Store receipt as ignored
                ...
                # CRITICAL FIX #2: Check if final status already set by another receipt with proper logging.
                cursor = await conn.execute(
                ...
                if processed and processed["result"] != receipt.result:
                    logger.warning(
                        f"CONFLICT receipt for operation {receipt.operation_id}: "
                        f"already has result {processed['result']}, ignoring {receipt.result}",
                        extra={
                            "operation_id": receipt.operation_id,
                ...
                await conn.rollback()
                # CRITICAL FIX #3: Return False instead of raising to avoid 500.
                logger.error(
